...
const UploadDocs = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/set_info`, {
        method: "POST",
        body: formData,
        headers: {
      ...
      if (result.results) {
        setFiles((prev) =>
          prev.map((file) => {
            ...
            // Only update status if file was in this upload batch
            if (fileResult) {
              return {
                ...file,
                status:
        ...
        // If no detailed results, mark uploaded files as uploaded
        setFiles((prev) =>
          prev.map((f) =>
