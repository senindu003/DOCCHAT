...
import { X, FileText, CheckCircle, AlertCircle, Loader, File, MessageCircle } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {files.length > 0 && <div className="bg-white rounded-xl shadow p-4 md:p-6 mb-6">
        ...
        <div className="space-y-3 max-h-100 overflow-y-auto pr-2">{files.map((file) => <div key={file.id} className={`p-3 md:p-4 border rounded-lg ${file.status === "uploaded" ? "border-green-200 bg-green-50" : file.status === "error" ? "border-red-200 bg-red-50" : file.status === "uploading" ? "border-blue-200 bg-blue-50" : "border-gray-200"}`}>
          ...
          {file.isPdf && <p className="text-xs text-amber-700 mt-3">Fast is best for normal text PDFs. It is not suitable for reliable image or table parsing—switch to High res for images, complex tables, or important layout.</p>}
        ...
        {uploadedCount > 0 && !isUploading && <div className="mt-4 flex justify-center"><button onClick={() => navigate("/chat")} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-blue-700"><MessageCircle className="h-4 w-4" />Chat with Docs</button></div>}
